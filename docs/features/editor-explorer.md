...
**Scratch (Untitled) buffer.** Every editor pane has one persistent **Untitled** scratch tab — what you type into when no file is open. It is modeled as a `Tab` keyed by the `UNTITLED` sentinel (`src/shared/editor-draft.ts`, the string `<untitled>` — invalid in Windows paths, so it never collides with a file key) with `saved = ''`, so the same draft machinery persists it (under `paneId::<untitled>` in `editor-drafts.json`) and restores it on launch with no new infrastructure. The tab shows when the pane has no file open or the buffer is non-empty (`• ` when non-empty); a `×` clears it (only shown when file tabs also exist). **Ctrl+S** (or the `Save As…` button) on the untitled tab runs `EditorPane.tsx:saveUntitledAs` → the new `dialog:saveFile` IPC (`dialog.showSaveDialog`, with a `TERMHALLA_SAVE_PATH` test hook) → `fsWrite` → opens the saved file as a normal tab and clears the scratch. An empty scratch buffer is never saved.
...
| `src/shared/editor-draft.ts` | pure `draftKey` + `resolveDraftOnOpen`; `UNTITLED` sentinel + `isUntitled` |
...
- **Read classification (finding 27, 2026-07 quality audit).** `readTextFile` never throws — it returns a discriminated `ReadResult` classified structurally in main: `ok` (with `tooLarge` — stats before reading; over `MAX_BYTES` (50 MB) it returns no content and the tab shows "File too large to open."), `binary` (NUL byte in the first 8000 — the tab shows "(binary)", the file exists), `not-found` (`ENOENT`/`ENOTDIR` only — the one state that renders "(deleted)"), and `error` (permissions, transient I/O, … — the tab shows a paint-only "(can't read)" marker plus the message in the pane body, never strikethrough). An errored tab blocks save (its empty buffer must never truncate a possibly-fine file), preserves any recovery draft, and self-heals when a watched `change` re-read succeeds. The renderer mapping is the pure `src/renderer/editor/read-state.ts`; a rejected invoke (handler teardown) also maps to errored, never missing.
...
- **Explorer context menu (UI polish phase 2).** Right-clicking a row opens `explorer-menu`: Open (files), **Reveal in File Explorer** (`fs:revealItem` → `shell.showItemInFolder` — distinct from the cwd-menu's `shell:reveal`/`openPath`, which would *open* a file rather than reveal it), **Copy path** / **Copy relative path** (`relativeTo(root, path)` + `clipboard:write`), **Rename** (inline edit reusing the workspace-rename focus pattern → `fs:rename`), and **Delete** (`fs:trash` → `shell.trashItem`, recoverable, behind a `window.confirm`). All disk mutations refresh the tree via the existing chokidar watch, never a manual re-read. fs failures surface as error toasts. Empty/errored expanded dirs render "Folder is empty" / "Couldn't read folder".
...
- **Main integration (vitest + temp dirs).** `tests/main/fs-files.test.ts` (write/read round-trip, `tooLarge` cap without reading content, structured `binary`/`not-found`/`error` classification, dirs-first listing, stat, pure `sortEntries`, `isBinary`); `tests/main/watch-manager.test.ts` (emits an `add` event for a new file in a watched dir; stops emitting after `unwatch`); `tests/renderer/read-state.test.ts` (pure `toTabReadState` mapping incl. the rejected-invoke `null` case).
- **Hot-exit + scratch (vitest).** `tests/shared/editor-draft.test.ts` (`draftKey`; `resolveDraftOnOpen` — passthrough, missing file, dirty vs unchanged baseline, external-change, deleted-with-draft, stale-equals-disk; plus the `UNTITLED` sentinel + `isUntitled`); `tests/main/draft-store.test.ts` (load-missing → `{}`, set/load round-trip, delete, malformed-entry sanitize, sync flush).
- **e2e (Playwright, hermetic seeded launch).** `tests/e2e/editor.spec.ts` — opens a seeded file in Monaco, edits + Ctrl+S writes disk; Ctrl+S after a tab switch; clean external change reloads; deleted file marks the tab. `tests/e2e/explorer.spec.ts` — explorer lists files, opens one into the editor with correct content, and reflects an externally-created file live. `tests/e2e/editor-hot-exit.spec.ts` — an unsaved draft is restored after relaunch (dirty marker shown), and a drafted file changed on disk while closed surfaces the reload bar; `tests/e2e/editor-hot-exit-midsession.spec.ts` — the draft is written to disk mid-session (no close). `tests/e2e/editor-scratch.spec.ts` — an untitled scratch buffer is restored after relaunch, and Ctrl+S Save As writes it to a real file.
