...
- **cmd heuristics:** `shellInjection` returns `null` for cmd, so it is detected purely by the output heuristics above; the busy/idle e2e genuinely depends on PowerShell markers because heuristics alone cannot distinguish a sleeping command from idle.
...
- `tests/main/status-tracker.test.ts` — `A→C→D;0→A` and failure exits, needs-input fire/clear, screen-redraw chunks not resetting quiet, and the heuristic/no-marker + nested-shell idle cases.
