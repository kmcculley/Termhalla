...
- **Merge conflicts (fixed 2026-07-09, baseline known bug #3).** Unmerged (`u`) entries are their own `conflicted` count — neither staged nor unstaged — shown in the popover as a `conflicted N` line (needs-attention tint) only while non-zero. A conflicted file still flips `dirty: true` and the dirty dot; resolved-and-staged files count as staged without being masked by the unmerged entries.
