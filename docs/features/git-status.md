...
- **Merge conflicts.** Unmerged (`u`) entries count toward `unstaged` only (never `staged`), so a conflicted file still flips `dirty: true` and the dirty dot, but the popover's staged/unstaged split does not separately surface conflict state. Acceptable for a read-only v1 indicator.
