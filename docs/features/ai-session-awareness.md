...
5. **Renderer.** `App.tsx` subscribes via `api.onAiSession` into `store.ts:setAiSession`, which maintains `aiSessions: Record<paneId, AiSession>` (delete-on-null; also dropped in `closePane`). The exported pure helper `store.ts:aiState(s, paneId)` derives `'working'` (status busy) / `'awaiting'` (else) / `null` (not an AI session). Surfaces:
...
| `src/main/ipc/register.ts` | Constructs `AiSessionTracker`; wires procs feed, command-done, unregister |
...
- **Awaiting = quiet, not gone.** Claude's prompt ends in `>`, so the generic status engine reads its quiet state as *idle*. For an active AI session the renderer reinterprets that as *awaiting input* (`aiState`), driving `✨⏳` and the notification.
...
- `tests/e2e/ai-session.spec.ts` — hermetic Playwright run: seeds a stub `claude.cmd` (busy, then waits on stdin), asserts the pane chip shows `✨ Claude` and the tab shows `✨`, then satisfies the stub's input so it exits and asserts the AI indicator clears (command-done).
