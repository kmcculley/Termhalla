...
| FINDING-DA-006 | LOW | A future Orky-side `v` bump that redefines (not just adds) a field's meaning would silently misrender under the current forward-compat-always-decode policy. Documented as a residual risk in the feature doc's drift-caveat section. |
