...
const api: TermhallaApi = {
  ...
  deleteWorkspace: (id) => ipcRenderer.invoke(CH.deleteWorkspace, id),
  pickWorkspaceSavePath: (defaultName) => ipcRenderer.invoke(CH.wsdocPickSave, defaultName),
  pickWorkspaceOpenPath: () => ipcRenderer.invoke(CH.wsdocPickOpen),
  readWorkspaceDoc: (path) => ipcRenderer.invoke(CH.wsdocRead, path),
  writeWorkspaceDoc: (path, contents) => ipcRenderer.invoke(CH.wsdocWrite, path, contents),
  loadWorkspaceDocPaths: () => ipcRenderer.invoke(CH.wsdocPathsLoad),
  setWorkspaceDocPath: (workspaceId, path) => ipcRenderer.send(CH.wsdocPathSet, workspaceId, path),
  clearWorkspaceDocPath: (workspaceId) => ipcRenderer.send(CH.wsdocPathClear, workspaceId),
  ...
  notesSet: (key, text) => ipcRenderer.invoke(CH.notesSet, key, text),
  ...
  orkyWatch: (id, cwd) => ipcRenderer.send(CH.orkyWatch, id, cwd),
  orkyUnwatch: (id) => ipcRenderer.send(CH.orkyUnwatch, id),
  onOrkyStatus: pushChannel<[string, import('@shared/types').OrkyPaneStatus | null]>(CH.orkyStatus),
  onRegistryStatus: pushChannel<[import('@shared/types').OrkyRegistrySnapshot]>(CH.registryStatus),
  registryCurrent: () => ipcRenderer.invoke(CH.registryCurrent),
  ...
  orkyDriveStatus: (req) => ipcRenderer.invoke(CH.orkyActionDriveStatus, req),
  ...
  onFileNew: pushChannel<[]>(CH.fileNew),
  onFileOpen: pushChannel<[]>(CH.fileOpen),
  onFileReopen: pushChannel<[]>(CH.fileReopen),
  onFileSave: pushChannel<[]>(CH.fileSave),
  onFileSaveAs: pushChannel<[]>(CH.fileSaveAs),
