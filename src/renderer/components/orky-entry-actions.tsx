...
export function useOrkyEntryActions(target: OrkyEntryTarget) {
   ...
   *  message the core classifier computed. */
 ...
 */
export function OrkyEntryActions({ target }: { target: OrkyEntryTarget }) {
  const { phase, binding, busy, bindAnswer, answerEscalation, answerReview, preview, resumeInTerminal, resetPhase } = useOrkyEntryActions(target)
  ...
  const errorView = phase.status === 'failure'
    ...
    : answerOpen && binding !== null && binding !== 'pending' && !binding.ok
